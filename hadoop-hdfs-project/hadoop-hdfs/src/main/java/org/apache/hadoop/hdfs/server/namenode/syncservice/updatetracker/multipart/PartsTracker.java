...
public class PartsTracker {
  ...
  private final Function<List<ByteBuffer>, MetadataSyncTask.MultipartCompleteMetadataSyncTask> createComplete;
  private CurrentTasks<BlockSyncTask> currentPutPartTasks;


  public PartsTracker(List<BlockSyncTask> putPartSyncTasks,
      Function<List<ByteBuffer>, MetadataSyncTask.MultipartCompleteMetadataSyncTask> createComplete,
  ...
  public Optional<CompleteTracker> markFinished(UUID syncTaskId, SyncTaskExecutionResult result) {
    ...
    if (this.uploadHandlesCollector.allPresent()) {
      return Optional.of(
          new CompleteTracker(
              this.createComplete.apply(this.uploadHandlesCollector.getCollectedHandles()),
              currentTasksFactory
          )
      );
    } else {
